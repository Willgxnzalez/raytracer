...
TEST(SphereTest, RayHitsSphereAtCenter) {
    Sphere sphere{Vec3{0, 0, 0}, 1.0, 0};
    Ray ray{Vec3{-5, 0, 0}, Vec3{1, 0, 0}};

    HitRecord record;
    bool hit = sphereHit(sphere, record, ray, 0.0, 100.0);
...
TEST(SphereTest, RayOriginatesInsideSphere) {
    Sphere sphere{Vec3{0, 0, 0}, 2.0, 0};
...
TEST(SphereTest, RayHitsWithinTRange) {
    ...
    bool hit = sphereHit(sphere, record, ray, 3.0, 5.0);
    EXPECT_TRUE(hit);
    
    // Hit is outside range
    hit = sphereHit(sphere, record, ray, 0.0, 3.0);
    EXPECT_FALSE(hit);
}
...
TEST(SphereTest, RejectsSelfIntersection) {
    ...
    EXPECT_FALSE(sphereHit(sphere, record, ray, 0.001, 100.0));
}
